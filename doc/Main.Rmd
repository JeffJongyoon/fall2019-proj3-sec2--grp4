...
**## ------>>>> Implementation of the baseline model starts here**
**## ------>>>> Implementation of the baseline model starts here**
**## ------>>>> Implementation of the baseline model starts here**
**## ------>>>> Implementation of the baseline model starts here**
```{r}
#learn the optimal parameters for the gbm model; reference: http://uc-r.github.io/gbm_regression
...
hyper_grid <- expand.grid(
  shrinkage = c(.01, .05,.1), #basically the learning rate
  interaction.depth = c(3, 5, 7), #maximum number of nodes per tree
  n.minobsinnode = 0,#c(5, 7)#, 10), #determines minimum number of features in each leaf of trees
  n_trees = c(50,100,500),#,50), #try with more trees this is just to check if it runs
  bag.fraction = 0,#c(.65, .8,1),#, 1),
...
```{r}
#using the best learned parameters, train the final model
best_params = NULL #!!!!fill in
tm_train=NA
tm_train <- system.time(fit_train <- train(dat_train, best_params))
save(fit_train, file="../output/fit_train.RData")


#test the final model !!!! SHOULD PASS IN OPTIMAL NUMBER OF TREES
source("../lib/test.R")
load(file="../output/feature_test.RData")
tm_test=NA
...
```{r}
#Nicole: Still working on this part
library(caret)
